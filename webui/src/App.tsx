...
function App() {
  ...
  useEffect(() => {
    juceBridge.whenReady(() => {
      ...
      //  第4引数の devicePixelRatio = 真のディスプレイ倍率。native の applyDisplayScale がこれを基準に
      //  Linux 埋め込み時のウィンドウ物理サイズ（transform = webViewDpr/peerScale）を補正する。
  ...
  //  VST3 の onSize 完了で解決）で、完了を待たず高頻度に送ると要求が積み上がってウィンドウが
  //  カーソルからどんどん遅れていく。「往復中は次を送らず、完了時に最新の保留サイズだけを送る」
  //  ことで host が捌ける最大レートで常に最新サイズだけを届け、蓄積遅延を無くす。
